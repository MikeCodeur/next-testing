...
Dans la section précédente, nous avons testé les services en `mockant` la persistance. Cela permet de tester la logique métier, autorisation, validation etc… Mais on ne peut pas détecter d’éventuels problèmes SQL, car rien n’est exécuté.

Plusieurs approches existent pour résoudre ce problème :

- Avoir une `BDD` dédiée à l’exécution des tests
- Avoir un utilitaire qui permet d’avoir une `BDD` proche de `Postgres` embarquée comme
  - [pg-mem](https://www.npmjs.com/package/pg-mem)
  - [test-container](https://testcontainers.com/modules/postgresql/)

Dans un premier temps, nous allons ici faire nos tests avec une `BDD` `Postgres` dédiée

- Etape 1 :

Nous allons donc nous assurer d’avoir une `URL` pour la base de tests `.env.test`
...
Il nous faut un fichier de configuration `drizzle` spécial pour les tests avec une initialisation des variables d’environnement pour l’environnement de tests.

Note : L’`URL` pourrait être mise en dur dans `url:` pour être sûr à 100% qu’un autre environnement n’est pas chargé
...
- Etape 3 : Avoir des scripts permettant d’avoir une base de données vide entre chaque test, par exemple :
...
Note : Ces fonctions seront fournies pour la suite de l’exercice.

## Exercice

Dans cet exercice, il y a un test qui permet de tester la création d’un utilisateur
...
Pour rappel : l’email doit être unique dans la table `email: text('email').unique()`
...
Ce qui veut dire que si un test s’exécute à nouveau, une erreur apparaitra.

```tsx
PostgresError: duplicate key value violates unique constraint "user_email_unique"
```

- **🐶**Dans cet exercice, tu vas devoir, dans un premier temps, appeler \*\*\*\*`initDrizzle` avant chaque test
  - `generateDb` (génère les scripts sql : `drizzle-kit generate`)
  - `migrateDb` (génère les scripts sql : `drizzle-kit migrate`)
