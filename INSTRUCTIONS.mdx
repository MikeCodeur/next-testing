...
Dans une application, il faut tester les différentes couches de notre application - Présentation - Service - Persistance. Il faut également tester le tout (Intégration / End to End) . Nous allons ici commencer par voir comment tester la présentation. C’est à dire les composants `React`. Nous utiliseront [Testing Librairie](https://testing-library.com/), la référence pour ce type de tests.

Pour tester les composants `React` avec `testing librairie` il faut utiliser [`render`](https://testing-library.com/docs/react-testing-library/api/#render) pour effectuer le rendu du composant , ensuite on peut tester avec [`screen`](https://testing-library.com/docs/queries/about/)
...
Dans cet exercice, tu vas devoir tester le composant `Home`.
...
Remplir le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20Mastery&entry.1430994900=8.Next%20Testing&entry.533578441=02%20Tests-component).
