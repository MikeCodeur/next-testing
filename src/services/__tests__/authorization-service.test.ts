...
describe('Authorization Service', () => {
  ...
  describe('checkRoleHierarchy', () => {
    it('should return true when user role is higher in hierarchy', () => {
      ...
      expect(checkRoleHierarchy(adminUser, RoleEnum.MODERATOR)).toBe(true)
    })

    it('should return true when user role is equal in hierarchy', () => {
      const moderatorUser = {
        id: faker.string.uuid(),
        email: faker.internet.email(),
      ...
      expect(checkRoleHierarchy(moderatorUser, RoleEnum.MODERATOR)).toBe(true)
    })
    ...
    it('should return false when user role is lower in hierarchy', () => {
      ...
      expect(checkRoleHierarchy(regularUser, RoleEnum.ADMIN)).toBe(false)
    })
    ...
    it('should default to USER role when no role provided', () => {
      ...
      expect(checkRoleHierarchy(userWithoutRole, RoleEnum.MODERATOR)).toBe(
        false
      )
    })
  })
